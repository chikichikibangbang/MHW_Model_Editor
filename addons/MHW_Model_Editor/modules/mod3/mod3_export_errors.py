import bpy
import textwrap
from ..common.general_function import textColors
...
class WM_OT_Mod3_ShowMHWMod3ErrorWindow(Operator):
    ...
    def draw(self, context):
        ...
        layout.label(
            text=f"The mod3 meshes has {len(self.errorList_items)} {'issues' if len(self.errorList_items) > 1 else 'issue'} that must be fixed before it can be exported.",
            icon="ERROR")
        # row = layout.row()
        if self.targetCollection != "":
            layout.label(text=f"Target Collection: {self.targetCollection}")
        if self.armatureName != "":
        ...
        if len(self.errorList_items) != 0:
            ...
            box = col2.box()
            box2 = col2.box()
            use_box2 = False  # 默认用box绘制

            for line in item.errorDescription.splitlines():
                line = line.strip()

                # 如果当前行是分隔行，则下一行切换到box2
                if "———" in line:
                    use_box2 = True
                    continue  # 跳过分隔行，不进行绘制

                # 按行宽拆分并绘制
                for chunk in textwrap.wrap(line, width=max_label_width):
                    box2.label(text=chunk) if use_box2 else box.label(text=chunk)
                    rowCount += 1
...
——————————————
HOW TO FIX:
Select a target mod3 collection in the export window.
""",
...
def showMHWMod3ErrorWindow(errorDict, targetCollectionName="", armatureName=""):
    ...
    bpy.ops.mhw_mod3.show_export_error_window('INVOKE_DEFAULT', targetCollection=targetCollectionName,
