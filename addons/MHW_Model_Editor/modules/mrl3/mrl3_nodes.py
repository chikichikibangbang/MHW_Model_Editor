...
def addPropertyNode(propValue, propName, currentPos, node_tree):
    ...
    if propNodeName in node_tree.nodes:
        ...
        if propName.endswith("__uiColor"):
            ...
            if propValue[1] == "float[3]":
                nodeGroup.inputs["Color"].default_value = propValue[0] + [1.0]
                nodeGroup.inputs["Alpha"].default_value = 1.0
            else:
                nodeGroup.inputs["Color"].default_value = propValue[0]
                nodeGroup.inputs["Alpha"].default_value = propValue[0][3]
            propNode = nodeGroup
        elif propValue[1] == "float[2]":
            # Vec2
            nodeGroup = getVec4NodeGroup(node_tree)
            nodeGroup.inputs[0].default_value = propValue[0][0]
            nodeGroup.inputs[1].default_value = propValue[0][1]
            nodeGroup.inputs[2].default_value = 0.0
            nodeGroup.inputs[3].default_value = 0.0
            propNode = nodeGroup
        elif propValue[1] == "float[3]":
            # Vec3
            nodeGroup = getVec4NodeGroup(node_tree)
            nodeGroup.inputs[0].default_value = propValue[0][0]
            nodeGroup.inputs[1].default_value = propValue[0][1]
            nodeGroup.inputs[2].default_value = propValue[0][2]
            nodeGroup.inputs[3].default_value = 0.0
            propNode = nodeGroup
        elif propValue[1] == "float[4]":
            # Vec4
            nodeGroup = getVec4NodeGroup(node_tree)
            nodeGroup.inputs[0].default_value = propValue[0][0]
            nodeGroup.inputs[1].default_value = propValue[0][1]
            nodeGroup.inputs[2].default_value = propValue[0][2]
            nodeGroup.inputs[3].default_value = propValue[0][3]
            propNode = nodeGroup
        else:
            propNode = node_tree.nodes.new("ShaderNodeValue")
            propNode.outputs["Value"].default_value = float(propValue[0])
